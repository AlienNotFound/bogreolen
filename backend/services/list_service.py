...
class ListService(BaseService):
    ...
    def add_to_list (user_id, book_id, listname):
        list = Liststb(user_id=user_id, book_id=book_id, listname=listname)
        db.session.add(list)
    ...
    def delete_book_from_lists(book_id, user_id):
        success, result = BaseService.delete_by_composite(Liststb, {"user_id": user_id, "book_id": book_id})

        return result if not success else None

    @staticmethod
    def get_list_by_user_and_book(user_id, book_id):
        return db.session.get(Liststb, {"user_id": user_id, "book_id": book_id})
    
    @staticmethod
    def get_lists_by_user(user_id):
        return BaseService.get_all_by_id(Liststb, Liststb.user_id, user_id)
    
    @staticmethod
    def get_book_status(user_id, book_id):
        return BaseService.get_by_composite(Liststb, {"user_id": user_id, "book_id": book_id})